...
retry_until 5 curl -kfsS --resolve "redirect.example.test:443:127.0.0.1" https://redirect.example.test/headers >/dev/null
echo "PASS: HTTPS endpoint serves traffic"

# Test 4: Follow redirect end-to-end
retry_until 5 curl -kLfsS --resolve "redirect.example.test:443:127.0.0.1" --resolve "redirect.example.test:80:127.0.0.1" -H 'Host: redirect.example.test' http://localhost/headers >/dev/null
