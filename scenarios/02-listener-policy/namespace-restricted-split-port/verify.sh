...
# Note: port 8080 is not mapped to the host in kind.config, so we can only
# verify via attachedRoutes, not actual traffic. Port 80 traffic would fail
# because the restricted listener rejects the cross-namespace route.
