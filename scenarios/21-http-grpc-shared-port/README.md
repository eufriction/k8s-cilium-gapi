...
Compared to [`20-http-grpc`](../20-http-grpc/README.md) (which uses separate ports `443` and `50051`), this scenario deliberately collapses everything onto **one port** to validate that Cilium can handle mixed HTTP/1.1 and gRPC (HTTP/2) traffic on the same HTTPS listener.
...
For the split-port variant, see [`scenarios/20-http-grpc`](../20-http-grpc/README.md).
