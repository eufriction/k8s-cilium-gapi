...
This scenario is the HTTPS counterpart to `01-http`. One Gateway terminates TLS on port `443` via a self-signed cert-manager certificate, with two HTTPRoute backends in separate namespaces (`https-a.example.test`, `https-b.example.test`).
...
This fills the gap between plaintext HTTP (`01-http`) and the gRPC/mTLS scenarios by establishing the simplest possible TLS-termination pattern. The Gateway terminates TLS on port `443`, which is exposed on your machine as `localhost:443` through kind `extraPortMappings`.
...
For the plaintext HTTP variant, see [`scenarios/01-http`](../01-http/README.md).
