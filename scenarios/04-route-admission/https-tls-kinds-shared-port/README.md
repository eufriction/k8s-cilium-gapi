# https-tls-kinds-shared-port — Kind restriction on mixed HTTPS + TLS passthrough
...
- [`https-tls-shared-port`](../../03-multi-listener/https-tls-shared-port/README.md) — same topology without explicit `allowedRoutes.kinds` (protocol implicitly determines allowed route type)

## Run

```sh
mise run //scenarios/04-route-admission/https-tls-kinds-shared-port:start
```
