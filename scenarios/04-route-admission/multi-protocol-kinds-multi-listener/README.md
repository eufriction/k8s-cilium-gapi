# multi-protocol-kinds-multi-listener — Per-listener kind restrictions across four protocols
...
- [`https-grpc-kinds-split-port`](../https-grpc-kinds-split-port/README.md) — per-listener kind restrictions on separate ports
- [`https-grpc-kinds-shared-port`](../https-grpc-kinds-shared-port/README.md) — both kinds on one listener, shared port
- [`http-https-tls-implicit-kinds`](../http-https-tls-implicit-kinds/README.md) — same protocol mix without explicit kind restrictions

## Run

```sh
mise run //scenarios/04-route-admission/multi-protocol-kinds-multi-listener:start
```
