# https-grpc-kinds-split-port — Per-listener `allowedRoutes.kinds` on separate ports
...
```sh
mise run //scenarios/04-route-admission/https-grpc-kinds-split-port:start
```

## See also

- [`03-multi-listener/http-grpc-split-port`](../../03-multi-listener/http-grpc-split-port/README.md) — same layout, no kind restrictions
- [`https-grpc-kinds-shared-port`](../https-grpc-kinds-shared-port/README.md) — shared port, both kinds on one listener
