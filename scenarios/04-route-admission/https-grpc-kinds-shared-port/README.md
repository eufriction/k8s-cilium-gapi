# https-grpc-kinds-shared-port — Single listener with multiple allowed route kinds
...
- [`03-multi-listener/http-grpc-shared-port`](../../03-multi-listener/http-grpc-shared-port/README.md) — same layout, no kind restrictions (implicit defaults)
- [`https-grpc-kinds-split-port`](../https-grpc-kinds-split-port/README.md) — one kind per listener on separate ports

## Run

```sh
mise run //scenarios/04-route-admission/https-grpc-kinds-shared-port:start
```
