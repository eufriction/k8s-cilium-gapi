...
skip_on_versions "1.19.1 1.19.3 1.20.0-pre.1" "catch-all HTTPS + TLS passthrough split-port broken (cilium#44889 + cilium#45371)"
...
# Tier 2 — gateway
kubectl wait gateway/https-tls-split-port-gateway -n gateway-system --for='jsonpath={.status.conditions[?(@.type=="Accepted")].status}=True' --timeout="${GW_READY_TIMEOUT:-30}s"
...
kubectl wait tlsroute/backend-b-tls-route -n backend-b --for='jsonpath={.status.parents[0].conditions[?(@.type=="ResolvedRefs")].status}=True' --timeout="${ROUTE_READY_TIMEOUT:-30}s" &
wait
