...
```sh
mise run //scenarios/03-multi-listener/http-grpc-shared-port:start
```

## See also

- [`http-grpc-split-port`](../http-grpc-split-port/README.md) — separate ports for HTTP and gRPC
- [`https-grpc-kinds-shared-port`](../../04-route-admission/https-grpc-kinds-shared-port/README.md) — shared-port variant with explicit `allowedRoutes.kinds`
