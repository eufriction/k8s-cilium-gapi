...
skip_on_versions "${SCENARIO_SKIP_VERSIONS:-}" "HTTPS + TLS passthrough same-hostname split-port broken (cilium#44889 + cilium#45371)"
...
assert_tls_isolation "https://api.example.test:${PORT_9443}/headers" \
  "HTTPRoute must not leak to TLS passthrough port (same hostname)" \
