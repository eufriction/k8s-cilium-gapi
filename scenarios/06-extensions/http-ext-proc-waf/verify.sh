...
metric_name="envoy_http_ext_proc_ceepf_backend_2da_coraza_2dwaf_streams_started"
...
baseline_metric=$(wait_for_ext_proc_metric_sum "$metric_name" "$metric_listener_prefix" "$metrics_timeout") || {
  ...
  if ! printf '%s\n' "$metrics" | grep -E 'ceepf_backend_2da_coraza_2dwaf|ext_proc|timed out waiting for the condition|ErrImagePull|ImagePullBackOff|CreateContainer|Pending' >&2; then
    [ -n "$metrics" ] && printf '%s\n' "$metrics" >&2
  fi
  exit 1
}
