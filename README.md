...
| `01-simple`          | [`http-grpc-split-port`](scenarios/01-simple/http-grpc-split-port/README.md)                                                       | HTTPS + gRPC on one gateway, separate ports, two namespaces                                                 | ⚠️ Listener collapse on all releases + #45623 on ≥1.19.2 (✅ combined) |
...
| `02-listener-policy` | [`kind-restricted-https-tls-shared-port`](scenarios/02-listener-policy/kind-restricted-https-tls-shared-port/README.md)            | HTTPS + TLS passthrough on port 443 with per-listener `allowedRoutes.kinds`                                 | ⚠️ #45559 on releases (✅ combined)                                    |
| `02-listener-policy` | [`kinds-multi-listener`](scenarios/02-listener-policy/kinds-multi-listener/README.md)                                              | 4 listeners (HTTP/HTTPS/gRPC/TLS), per-listener `allowedRoutes.kinds`, HTTP→HTTPS redirect, TLS passthrough | ⚠️ #45559 on releases (✅ combined)                                    |
...
| `02-listener-policy` | [`kinds-split-port`](scenarios/02-listener-policy/kinds-split-port/README.md)                                                      | HTTPS + gRPC on separate ports with per-listener `allowedRoutes.kinds`                                      | ⚠️ #45559 + #45623 on releases (✅ combined)                           |
...
| `02-listener-policy` | [`no-sectionname`](scenarios/02-listener-policy/no-sectionname/README.md)                                                          | TLSRoute without sectionName on mixed-listener Gateway (HTTP/HTTPS/TLS)                                     | ✅ Pass (#45050 fixed in #45371, merged to main)                       |
| `03-multi-port`      | [`http-grpc-same-hostname`](scenarios/03-multi-port/http-grpc-same-hostname/README.md)                                             | HTTPRoute + GRPCRoute on same hostname, different ports (443 / 50051)                                       | ⚠️ #44877 + #45623 + listener collapse (same hostname) on all versions |
| `03-multi-port`      | [`https-tls-same-hostname-split-port`](scenarios/03-multi-port/https-tls-same-hostname-split-port/README.md)                       | HTTPS termination + TLS passthrough, same hostname, different ports                                         | ⚠️ #42898                                                              |
| `03-multi-port`      | [`kind-restricted-https-tls-split-port`](scenarios/03-multi-port/kind-restricted-https-tls-split-port/README.md)                   | Kind-restricted HTTPS + TLS passthrough on split ports with `allowedRoutes.kinds`                           | ⚠️ #42898                                                              |
| `03-multi-port`      | [`namespace-restricted-same-hostname-split-port`](scenarios/03-multi-port/namespace-restricted-same-hostname-split-port/README.md) | Namespace-restricted same-hostname split-port with `allowedRoutes.namespaces`                               | ⚠️ #42159 + #44877 + listener collapse (same hostname) on all versions |
...
> **Note:** Issue IDs are suspected causes based on observed symptoms. "✅ combined" means verified passing on the `fix/combined-gateway-fixes` branch (all open fixes cherry-picked together). See the [Known Cilium bugs](#known-cilium-bugs) table below for details.
...
| ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CheckGatewayAllowedForNamespace` doesn't enforce per-listener namespace restrictions             | namespace-restricted-same-hostname-split-port                                                           | [#42159](https://github.com/cilium/cilium/issues/42159)                                        | [fix/allowed-routes-main](https://github.com/cilium/cilium/tree/fix/allowed-routes-main) | Broken on all releases; ✅ on combined branch                                                                                                        |
| `CheckGatewayRouteKindAllowed` overwrites Accepted condition across listeners                     | kinds-split-port, kinds-multi-listener, kind-restricted-https-tls-shared-port                           | [#45559](https://github.com/cilium/cilium/issues/45559)                                        | [fix/allowed-routes-main](https://github.com/cilium/cilium/tree/fix/allowed-routes-main) | Broken on all releases; ✅ on combined branch (all 3 scenarios pass)                                                                                 |
| TLS passthrough split-port same-hostname Envoy wiring failure                                     | tls-passthrough-same-hostname, https-tls-same-hostname-split-port, kind-restricted-https-tls-split-port | [#42898](https://github.com/cilium/cilium/issues/42898)                                        | —                                                                                        | Broken on all tested versions including combined branch (`SSL_ERROR_SYSCALL`); the only remaining open bug — causes the 3 remaining ❌ in combined-2 |
| `toFilterChainMatch` duplicate `serverNames` after `SortedUnique` removal                         | http-grpc-split-port, kinds-split-port, http-grpc-same-hostname                                         | [#45623](https://github.com/cilium/cilium/issues/45623)                                        | [fix/server-names-dedup](https://github.com/cilium/cilium/tree/fix/server-names-dedup)   | Broken on ≥1.19.2 (regression in `9ee2db2b32`); works on 1.19.1; ✅ on combined branch                                                               |
| Multi-port HTTPS listener collapse — routes leak across ports (different hostnames)               | http-grpc-split-port, kinds-split-port                                                                  | — (needs filing; data-plane aspect of [#42159](https://github.com/cilium/cilium/issues/42159)) | per-port listener commits on combined branch                                             | Broken on all releases; ✅ on combined branch (per-port Envoy listeners)                                                                             |
| Multi-port HTTPS same-hostname — per-port route config doesn't respect `allowedRoutes.namespaces` | namespace-restricted-same-hostname-split-port                                                           | — (data-plane aspect of [#42159](https://github.com/cilium/cilium/issues/42159))               | —                                                                                        | Broken on all versions including #44889; per-port route configs include all hostname-matching routes regardless of listener acceptance               |
| Same-hostname GRPCRoutes on split ports return 404                                                | http-grpc-same-hostname, namespace-restricted-same-hostname-split-port                                  | [#44877](https://github.com/cilium/cilium/issues/44877)                                        | [#44889](https://github.com/cilium/cilium/pull/44889)                                    | Broken on ≤1.20.0-pre.1; ✅ on combined branch                                                                                                       |

#### Fixed bugs

| Bug                                                                                   | Scenarios                           | Cilium issue                                            | Fix                                                   | Available since                                           |
| ------------------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------- | ----------------------------------------------------- | --------------------------------------------------------- |
| TLSRoute without sectionName creates duplicate FilterChains on mixed-listener Gateway | no-sectionname                      | [#45050](https://github.com/cilium/cilium/issues/45050) | [#45371](https://github.com/cilium/cilium/pull/45371) | Merged to main (commits `24022e316a`, `22c2bfb133`)       |
...
| Group                | Scenario                                      | 1.19.1 | 1.19.3 | 1.20.0-pre.1 | #44889 |
| -------------------- | --------------------------------------------- | :----: | :----: | :----------: | :----: |
| `01-simple`          | grpc                                          |   ✅   |   ✅   |      ✅      |   ✅   |
| `01-simple`          | http                                          |   ✅   |   ✅   |      ✅      |   ✅   |
