...
| `01-simple`          | [`http-grpc-split-port`](scenarios/01-simple/http-grpc-split-port/README.md)                                                       | HTTPS + gRPC on one gateway, separate ports, two namespaces                                                 | ⚠️ Listener collapse + #45623 on releases (✅ #44889)                            |
...
| `02-listener-policy` | [`kind-restricted-https-tls-shared-port`](scenarios/02-listener-policy/kind-restricted-https-tls-shared-port/README.md)            | HTTPS + TLS passthrough on port 443 with per-listener `allowedRoutes.kinds`                                 | ⚠️ #45559 on all tested versions incl. #44889                                    |
| `02-listener-policy` | [`kinds-multi-listener`](scenarios/02-listener-policy/kinds-multi-listener/README.md)                                              | 4 listeners (HTTP/HTTPS/gRPC/TLS), per-listener `allowedRoutes.kinds`, HTTP→HTTPS redirect, TLS passthrough | ⚠️ #45559 on all tested versions incl. #44889                                    |
...
| `02-listener-policy` | [`kinds-split-port`](scenarios/02-listener-policy/kinds-split-port/README.md)                                                      | HTTPS + gRPC on separate ports with per-listener `allowedRoutes.kinds`                                      | ⚠️ #45559 on all tested versions incl. #44889 (listener collapse + #45623 fixed) |
...
| `03-multi-port`      | [`http-grpc-same-hostname`](scenarios/03-multi-port/http-grpc-same-hostname/README.md)                                             | HTTPRoute + GRPCRoute on same hostname, different ports (443 / 50051)                                       | ⚠️ #44877 + #45623 on releases (✅ #44889)                                       |
...
| `03-multi-port`      | [`namespace-restricted-same-hostname-split-port`](scenarios/03-multi-port/namespace-restricted-same-hostname-split-port/README.md) | Namespace-restricted same-hostname split-port with `allowedRoutes.namespaces`                               | ⚠️ #42159 on all versions incl. #44889 (#44877 fixed)                            |
...
> **Note:** Issue IDs are suspected causes based on observed symptoms. "✅ #44889" means verified passing on the [`fix/gateway-api-same-hostname-split-ports`](https://github.com/cilium/cilium/pull/44889) branch (per-port listener separation). See the [Known Cilium bugs](#known-cilium-bugs) table below for details.
